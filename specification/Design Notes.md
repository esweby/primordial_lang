...
Both of these constructs will be immutable by default but their internal content will be mutable by default using internal methods and direct access.

These are common rules for arrays and slices

- They do not support nil values, any value set will be the values neutral value
- Any assignment of a target to another declaration will be a deep copy, the assignee will inherit the targets type
  - Any copies will be transformed to a slice
  - If you declare the variable with a type different to the type you are copying from you will get an error
...
- Slices may not contain a nil value
  - Assigning a middle value nil would resut in an error being thrown
...
An entry can be accessed using bracket notation, like other languages.
